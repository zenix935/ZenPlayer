...
bool VlcEngine::init(const QString& vlcLibPath)
{
    ...
    // Load libvlc.dll
    QString dllPath = QDir(vlcLibPath).absoluteFilePath("libvlc.dll");
    ...
    QString pluginPath = QDir(vlcLibPath).absoluteFilePath("plugins");
...
VlcEngine::State VlcEngine::state() const
{
    return m_state;
}
...
void VlcEngine::vlcEventCallback(const libvlc_event_t* event, void* userData)
{
    ...
    QMetaObject::invokeMethod(self, [self, eventType]() {
        self->handleVlcEventType(eventType);
    }, Qt::QueuedConnection);
}
